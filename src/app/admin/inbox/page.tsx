  ...
  return (
    <div className="flex flex-col h-[calc(100vh-60px)] max-w-[1500px] mx-auto relative z-10 pb-6 gap-6">
      ...
      <div className="flex gap-6 flex-1 min-h-0 animate-[fadeInUp_0.8s_ease-out_0.2s_both]">
          ...
          <div className="flex-1 overflow-y-auto custom-scrollbar p-4 flex flex-col gap-6">
            ...
            <div className="flex flex-col gap-2">
              ...
              {channels.filter(c => !c.is_private && !c.is_archived).length === 0 && (
                 <div className="px-2 text-[10px] font-roboto text-[var(--color-atelier-grafite)]/40 italic">Nenhum canal ativo.</div>
              )}
        ...
        {/* 🛠️ CORREÇÃO CRÍTICA: Adicionado `min-h-0 min-w-0` para confinar o flexbox e permitir o scroll interno */}
        <div className="flex-1 min-h-0 min-w-0 glass-panel rounded-[2.5rem] bg-white/60 border border-white flex flex-col relative overflow-hidden shadow-[0_20px_50px_rgba(122,116,112,0.08)] h-full">
            ...
            {activeChannel && (
              <div className="relative">
                <button 
                  ...
                  className="w-10 h-10 rounded-xl bg-white border border-white hover:border-[var(--color-atelier-grafite)]/20 flex items-center justify-center text-[var(--color-atelier-grafite)]/50 transition-all shadow-sm"
                >
                  <Settings2 size={16} />
                </button>
                ...
                <AnimatePresence>
                  {isChannelSettingsOpen && (
                    <motion.div 
                      ...
                      className="absolute right-0 top-12 w-48 bg-white border border-[var(--color-atelier-grafite)]/10 rounded-2xl shadow-xl z-50 flex flex-col py-2 overflow-hidden"
                    >
                      <button onClick={handleArchiveChannel} className="w-full text-left px-4 py-2.5 flex items-center gap-2 font-roboto text-[12px] font-bold text-[var(--color-atelier-grafite)] hover:bg-[var(--color-atelier-grafite)]/5 transition-colors">
                        <Archive size={14}/> Arquivar Canal
                      </button>
                      <button onClick={handleDeleteChannel} className="w-full text-left px-4 py-2.5 flex items-center gap-2 font-roboto text-[12px] font-bold text-red-500 hover:bg-red-50 transition-colors">
          ...
          <div className="flex-1 overflow-y-auto custom-scrollbar px-8 py-8 flex flex-col gap-6 bg-gradient-to-b from-transparent to-white/30">
            ...
            {/* 🛠️ CORREÇÃO CRÍTICA: Removido mode="popLayout" para não rebentar a geometria das bolhas de chat na animação */}
            <AnimatePresence>
