...
import "./globals.css";
import { supabase } from "../lib/supabase"; // Importe o Supabase para puxar o nome/foto reais
...
export default function RootLayout({ children }: { children: React.ReactNode }) {
  ...
  // Estado para armazenar os dados reais do perfil incluindo Empresa e Cargo
  ...
  useEffect(() => {
    const checkAuthAndProfile = async () => {
      ...
      if (token && role) {
        ...
        // Puxa o perfil real do banco de dados silenciosamente (incluindo empresa e cargo)
  ...
  const handleLogout = () => {
    ...
    setTimeout(() => {
      localStorage.removeItem("atelier_token");
      localStorage.removeItem("atelier_role");
      supabase.auth.signOut(); // Desloga do banco também
      router.replace('/login');
    }, 1000);
  };

  // ==========================================
  // ECRÃ DE CARREGAMENTO IMERSIVO (NOVO PRELOADER)
  // ==========================================
