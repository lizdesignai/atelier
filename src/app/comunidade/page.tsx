...
import { 
  ...
  MoreVertical, Loader2, User, ImageIcon, CheckCircle2, AlertCircle
} from "lucide-react";
...
// Missão 3.3: Gamificação focada na aplicação da Marca
const BADGES = [
  { id: 1, name: "Unboxing Oficial", desc: "Mostrou a primeira embalagem impressa.", icon: <Star size={16} />, active: true },
  { id: 2, name: "Sinalização Física", desc: "Aplicou a marca na fachada ou interior.", icon: <Globe2 size={16} />, active: true },
  { id: 3, name: "Domínio Digital", desc: "Site ou Feed totalmente reestruturado.", icon: <Award size={16} />, active: false },
  { id: 4, name: "Legado Atemporal", desc: "1 ano de consistência visual.", icon: <Trophy size={16} />, active: false },
];
...
export default function ComunidadePage() {
  ...
  const fetchCommunityData = async () => {
    try {
      ...
      // Busca Feed da Comunidade (O SQL RLS já filtra os aprovados ou pendentes do próprio usuário)
      // Se a tabela community_posts AINDA NÃO TEM a coluna 'status', isto vai dar erro. 
      // Certifique-se que rodou o SQL do passo 1.
  ...
  const handlePublish = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // 2. Insere o Post (Por padrão, o banco de dados vai defini-lo como 'pending')
      const { data: insertedPost, error: dbError } = await supabase
        ...
        .insert({
          ...
          image_url: imageUrl,
          status: userProfile.role === 'admin' ? 'approved' : 'pending' // Admin aprova direto
        })
        .select('*, profiles(nome, avatar_url, role)');
      ...
      // Scrola para o topo para ver o novo post
      feedRef.current?.scrollTo({ top: 0, behavior: 'smooth' });
      
      if (userProfile.role !== 'admin') {
        showToast("Publicação enviada para análise do Atelier. Ganhará 150 EXP após aprovação!");
      } else {
  ...
  // Função para o Admin aprovar um post pendente
      ...
      <div className="flex gap-6 flex-1 min-h-0 animate-[fadeInUp_0.8s_ease-out_0.2s_both]">
        ...
        <div className="w-[350px] flex flex-col gap-6 h-full shrink-0">
          ...
          <div className="glass-panel p-8 rounded-[2.5rem] bg-gradient-to-br from-[var(--color-atelier-grafite)] to-[#2a2826] text-white shadow-[0_20px_50px_rgba(122,116,112,0.15)] flex flex-col items-center text-center relative overflow-hidden shrink-0">
            ...
            <span className="font-roboto text-[10px] uppercase tracking-widest font-bold text-white/50 mb-6">
              {userProfile?.role === 'admin' ? 'Fundadora' : 'Membro Premium'}
            </span>
            ...
            <div className="w-full flex flex-col gap-2">
              ...
              <p className="font-roboto text-[9px] text-white/40 mt-1">Sua conta ganha +150 EXP por cada partilha aprovada.</p>
            </div>
          </div>

          {/* O Cofre de Conquistas (Medalhas) */}
          <div className="glass-panel p-6 rounded-[2.5rem] bg-white/60 border border-white flex-1 flex flex-col min-h-0 shadow-[0_10px_30px_rgba(122,116,112,0.05)]">
            <div className="flex items-center justify-between mb-4">
              <h3 className="font-elegant text-xl text-[var(--color-atelier-grafite)] flex items-center gap-2">
                <Medal size={18} className="text-[var(--color-atelier-terracota)]" /> Missões da Marca
              </h3>
              ...
              {BADGES.map(badge => (
                <div 
                  ...
                  className={`
                    ...
                    ${badge.active ? 'bg-white border-[var(--color-atelier-grafite)]/10 shadow-sm' : 'bg-white/30 border-transparent opacity-60 grayscale'}
                  `}
                >
                  <div className={`w-10 h-10 rounded-full flex items-center justify-center shrink-0 shadow-sm ${badge.active ? 'bg-[var(--color-atelier-terracota)] text-white' : 'bg-[var(--color-atelier-grafite)]/10 text-[var(--color-atelier-grafite)]'}`}>
                    {badge.icon}
                  </div>
                  <div className="flex flex-col">
                    ...
                    <span className="font-roboto text-[9px] uppercase tracking-widest text-[var(--color-atelier-grafite)]/50 mt-0.5 leading-tight">{badge.desc}</span>
                  </div>
                </div>
              ))}
            </div>
        ...
        {/* COLUNA DIREITA: O FEED E O PUBLICADOR NA BASE (65%) */}
        {/* Missão 3.2: Reengenharia UI - Publicador na Base */}
        <div className="flex-1 flex flex-col h-full min-h-0 relative">
                  ...
                  >
                    <div className="flex justify-between items-start">
                      ...
                      {isPending && isAdmin ? (
                        ...
                        <button className="text-[var(--color-atelier-grafite)]/30 hover:text-[var(--color-atelier-terracota)] transition-colors"><MoreVertical size={16} /></button>
                      )}
                    ...
                    <div className="flex items-center justify-between pt-4 border-t border-[var(--color-atelier-grafite)]/5 mt-2 px-1">
                      <div className="flex gap-2">
                        <button 
                          ...
                          disabled={isPending}
                          className="px-4 py-2 rounded-xl bg-white/60 hover:bg-white border border-white shadow-sm flex items-center gap-2 font-roboto text-[11px] font-bold text-[var(--color-atelier-grafite)]/70 hover:text-[var(--color-atelier-terracota)] transition-colors disabled:opacity-50"
                        >
          ...
          {/* O Publicador "Bottom-Fixed" (Estilo Chat) */}
          <div className="glass-panel p-4 rounded-[2rem] bg-white/90 border border-white shadow-[0_-10px_40px_rgba(122,116,112,0.1)] shrink-0 z-20 mt-4">
            ...
            {newPostImagePreview && (
              <div className="relative w-24 h-24 rounded-xl overflow-hidden border-2 border-white shadow-sm mb-3 ml-2">
                ...
                <button type="button" onClick={() => {setNewPostImagePreview(null); setNewPostImageFile(null)}} className="absolute top-1 right-1 w-6 h-6 rounded-full bg-black/50 text-white flex items-center justify-center hover:bg-red-500 transition-colors text-xs backdrop-blur-sm">
                  ✕
                </button>
              </div>
            )}
            ...
            <form onSubmit={handlePublish} className="flex items-center gap-3">
              ...
              <input 
                ...
                placeholder="Partilhe a vitória da sua marca. Como ela respira hoje no mundo real?"
                className="flex-1 bg-transparent border-none outline-none font-roboto text-[14px] text-[var(--color-atelier-grafite)] placeholder:text-[var(--color-atelier-grafite)]/40 px-2 disabled:opacity-50"
              />

              <button 
                type="submit" 
                disabled={!newPostText.trim() || isPublishing}
                className="w-12 h-12 flex items-center justify-center rounded-full bg-[var(--color-atelier-terracota)] text-white hover:bg-[#8c562e] transition-transform shadow-md disabled:opacity-50 disabled:bg-[var(--color-atelier-grafite)]/10 disabled:text-[var(--color-atelier-grafite)]/30 shrink-0 hover:-translate-y-0.5"
              >
