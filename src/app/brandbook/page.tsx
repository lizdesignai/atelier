...
import { 
  ...
  Download, Sparkles, X, ChevronRight, Info
} from "lucide-react";
...
if (isLoading) return <div className="flex h-screen w-full items-center justify-center">...</div>;
  ...
  return (
    <div className="flex flex-col h-full flex-1 w-full relative z-10 overflow-hidden text-[var(--color-atelier-grafite)]">
      ...
      <header className="py-4 md:py-6 flex justify-between items-center shrink-0 z-20">
        <div>
          ...
          <h1 className="font-elegant text-2xl md:text-3xl text-[var(--color-atelier-grafite)] leading-none">O Brandbook Vivo</h1>
        </div>
        <button onClick={() => router.push('/cockpit')} className="w-10 h-10 rounded-xl border border-[var(--color-atelier-grafite)]/10 flex items-center justify-center text-[var(--color-atelier-grafite)]/50 hover:bg-white hover:text-[var(--color-atelier-terracota)] transition-all shadow-sm">
          <X size={18} />
        </button>
      </header>
      ...
      <div className="flex flex-1 min-h-0 relative gap-6">
        ...
        <div className="w-[260px] hidden md:flex flex-col shrink-0 z-10 h-full border-r border-[var(--color-atelier-grafite)]/5 pr-4">
          <div className="py-2">
            <p className="font-roboto text-[11px] text-[var(--color-atelier-grafite)]/60 leading-relaxed mb-6 font-medium">
              ...
              {[
                ...
                >
                  <div className="flex items-center gap-3">
                    <div className={`w-8 h-8 rounded-xl flex items-center justify-center ${activeZone === zone.id ? 'bg-[var(--color-atelier-terracota)]/10 text-[var(--color-atelier-terracota)]' : 'bg-[var(--color-atelier-grafite)]/5'}`}>
                      {zone.icon}
                    </div>
          ...
          <div className="mt-auto py-4">
            <button 
              ...
              className={`w-full py-4 rounded-2xl font-bold uppercase tracking-widest text-[10px] flex items-center justify-center gap-2 transition-all shadow-md hover:-translate-y-0.5
                ${activeZone === 5 ? 'bg-white text-[var(--color-atelier-grafite)] border border-[var(--color-atelier-grafite)]/10' : 'bg-[var(--color-atelier-grafite)] text-white hover:bg-[var(--color-atelier-terracota)]'}
              `}
            >
        ...
        <div className="flex-1 relative overflow-y-auto custom-scrollbar px-2 pb-20 pt-2 scroll-smooth flex justify-center items-start">
          <AnimatePresence mode="wait">
            ...
            {/* =========================================================================
                ...
                ========================================================================= */}
            {activeZone === 1 && (
              <motion.div key="z1" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} className="w-full max-w-2xl flex flex-col gap-4 pb-4 h-full justify-center">
                <div className="text-center shrink-0">
                  <h2 className="font-elegant text-3xl text-[var(--color-atelier-grafite)] mb-2">O Eixo de Atenção</h2>
                  <p className="font-roboto text-[12px] text-[var(--color-atelier-grafite)]/70 leading-relaxed max-w-xl mx-auto">
                ...
                <div className="glass-panel bg-white/80 p-6 md:p-8 rounded-[2.5rem] border border-white shadow-[0_20px_50px_rgba(0,0,0,0.03)] flex flex-col gap-6">
                  
                  <div className="flex items-center justify-center shrink-0">
                    <div className="bg-[var(--color-atelier-creme)]/50 border border-[var(--color-atelier-grafite)]/10 px-6 py-2.5 rounded-full flex items-center gap-4 shadow-inner">
                      ...
                      <span className="font-elegant text-3xl text-[var(--color-atelier-terracota)] leading-none">{totalTilt}<span className="text-sm text-[var(--color-atelier-grafite)]/30">/100</span></span>
                    </div>
                  </div>
                      ...
                      <div key={pillar.key}>
                        <div className="flex justify-between items-end mb-2">
                          <div className="pr-4">
                            ...
                            <p className="text-[10px] text-[var(--color-atelier-grafite)]/50 mt-0.5 leading-relaxed">{pillar.desc}</p>
                          </div>
                          <span className="font-elegant text-2xl text-[var(--color-atelier-terracota)] shrink-0 leading-none">{tilt[pillar.key as keyof typeof tilt]}%</span>
                        </div>
                        <input type="range" min="0" max="100" value={tilt[pillar.key as keyof typeof tilt]} onChange={(e) => handleTiltChange(pillar.key as keyof typeof tilt, parseInt(e.target.value))} className="w-full h-2 bg-[var(--color-atelier-grafite)]/10 rounded-full appearance-none cursor-pointer accent-[var(--color-atelier-terracota)] shadow-inner" />
                      </div>
                    ))}
                  </div>
                  
                  <button onClick={() => { setActiveZone(2); saveProgress(); }} className="mt-2 px-8 py-4 bg-[var(--color-atelier-terracota)] text-white rounded-2xl font-bold uppercase tracking-widest text-[11px] hover:bg-[#8c562e] transition-colors shadow-md flex items-center justify-center gap-2 shrink-0">
            ...
            {activeZone === 2 && (
              <motion.div key="z2" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} className="w-full max-w-4xl flex flex-col gap-8 pb-10 h-full">
                <div className="text-center shrink-0">
                  ...
                  <p className="font-roboto text-[13px] text-[var(--color-atelier-grafite)]/70 leading-relaxed max-w-xl mx-auto">
                ...
                <div className="flex-1 flex flex-col items-center justify-center min-h-[500px]">
                  <div className="flex items-center gap-2 mb-6">
                    {SEMIOTICS_PAIRS.map((_, i) => (
                      <div key={i} className={`w-2 h-2 rounded-full transition-all ${i === activeSemioticsIndex ? 'bg-[var(--color-atelier-terracota)] w-6' : i < activeSemioticsIndex ? 'bg-[var(--color-atelier-terracota)]/40' : 'bg-gray-200'}`}></div>
                    ))}
                  </div>
                  ...
                  <AnimatePresence mode="wait">
                    {SEMIOTICS_PAIRS[activeSemioticsIndex] && (
                      <motion.div key={activeSemioticsIndex} initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 1.05 }} transition={{ duration: 0.3 }} className="w-full flex flex-col gap-6">
                        <h3 className="font-roboto text-[10px] font-bold uppercase tracking-widest text-[var(--color-atelier-terracota)] text-center mb-2">
                        ...
                        <div className="flex flex-col md:flex-row gap-6 md:gap-10 items-stretch">
                          ...
                          >
                            <div className={`w-full aspect-[4/3] md:aspect-square rounded-[3rem] overflow-hidden shadow-lg transition-all duration-300 border-4 ${semiotics[SEMIOTICS_PAIRS[activeSemioticsIndex].id] === 'A' ? 'border-[var(--color-atelier-terracota)] scale-[1.02]' : 'border-white group-hover:shadow-2xl'}`}>
                            ...
                            <p className="font-roboto text-[11px] font-medium text-[var(--color-atelier-grafite)]/70 mt-4 text-center px-4 leading-relaxed">
                          ...
                          <button 
                            ...
                            className="flex-1 flex flex-col group relative outline-none text-left"
                          >
                            <div className={`w-full aspect-[4/3] md:aspect-square rounded-[3rem] overflow-hidden shadow-lg transition-all duration-300 border-4 ${semiotics[SEMIOTICS_PAIRS[activeSemioticsIndex].id] === 'B' ? 'border-[var(--color-atelier-terracota)] scale-[1.02]' : 'border-white group-hover:shadow-2xl'}`}>
            ...
            {/* =========================================================================
                ...
                ========================================================================= */}
            {activeZone === 3 && (
              <motion.div key="z3" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} className="max-w-3xl mx-auto flex flex-col gap-4 pb-4 h-full justify-center">
                <div className="text-center shrink-0">
                ...
                <div className="flex-1 flex flex-col items-center justify-center min-h-0 w-full">
                  <div className="flex items-center gap-2 mb-4 shrink-0">
                    {SCENARIOS.map((_, i) => (
                      <div key={i} className={`w-2 h-2 rounded-full transition-all ${i === activeVoiceIndex ? 'bg-[var(--color-atelier-terracota)] w-6' : i < activeVoiceIndex ? 'bg-[var(--color-atelier-terracota)]/40' : 'bg-gray-200'}`}></div>
                    ))}
                  </div>

                  <AnimatePresence mode="wait">
                    {SCENARIOS[activeVoiceIndex] && (
                      <motion.div key={activeVoiceIndex} initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} transition={{ duration: 0.3 }} className="w-full glass-panel bg-white/80 p-6 md:p-8 rounded-[2.5rem] border border-white shadow-[0_20px_50px_rgba(0,0,0,0.03)] flex flex-col">
                        
                        <div className="mb-6 pb-6 border-b border-[var(--color-atelier-grafite)]/10 text-center shrink-0">
                          <span className="font-roboto text-[10px] uppercase font-bold tracking-widest text-[var(--color-atelier-terracota)] mb-2 block">Cenário {activeVoiceIndex + 1}: {SCENARIOS[activeVoiceIndex].title}</span>
                          <p className="font-elegant text-xl text-[var(--color-atelier-grafite)] italic leading-relaxed">"{SCENARIOS[activeVoiceIndex].context}"</p>
                        </div>
                        ...
                        <div className="flex flex-col gap-3">
                          {SCENARIOS[activeVoiceIndex].options.map(opt => (
                            <button 
                              ...
                              className={`p-4 rounded-2xl cursor-pointer transition-all border flex gap-4 items-center text-left
                                ${voice[SCENARIOS[activeVoiceIndex].id] === opt.id ? 'bg-[var(--color-atelier-terracota)]/5 border-[var(--color-atelier-terracota)]/40 ring-1 ring-[var(--color-atelier-terracota)]/20 shadow-sm scale-[1.01]' : 'bg-white border-[var(--color-atelier-grafite)]/10 hover:border-[var(--color-atelier-terracota)]/30'}
                              `}
                            >
                              <div className={`w-6 h-6 rounded-full flex items-center justify-center shrink-0 border-2 transition-colors
                                ${voice[SCENARIOS[activeVoiceIndex].id] === opt.id ? 'bg-[var(--color-atelier-terracota)] border-[var(--color-atelier-terracota)] text-white' : 'bg-transparent border-[var(--color-atelier-grafite)]/20'}
                              `}>
                                {voice[SCENARIOS[activeVoiceIndex].id] === opt.id && <CheckCircle2 size={12} />}
                              </div>
                              <div>
                                <span className={`block font-roboto text-[10px] uppercase tracking-widest font-bold mb-1 ${voice[SCENARIOS[activeVoiceIndex].id] === opt.id ? 'text-[var(--color-atelier-terracota)]' : 'text-[var(--color-atelier-grafite)]/50'}`}>{opt.label}</span>
                                <p className="text-[12px] text-[var(--color-atelier-grafite)] leading-relaxed font-medium">{opt.text}</p>
                              </div>
                            </button>
                          ))}
                        </div>
            ...
            {activeZone === 4 && (
              <motion.div key="z4" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} className="max-w-4xl mx-auto flex flex-col gap-8 pb-10 min-h-full">
                ...
                <div className="flex-1 flex flex-col gap-8">
                  <label className="w-full bg-white/60 border-2 border-dashed border-[var(--color-atelier-terracota)]/30 hover:border-[var(--color-atelier-terracota)] hover:bg-white rounded-[3rem] p-16 flex flex-col items-center justify-center cursor-pointer transition-all shadow-sm group">
                    ...
                    {isUploading ? (
                      <div className="flex flex-col items-center gap-4 text-[var(--color-atelier-terracota)]">
                        ...
                        <span className="font-roboto text-[11px] font-bold uppercase tracking-widest">A encriptar sinapse visual...</span>
                      </div>
                    ) : (
                      <>
                        <div className="w-20 h-20 rounded-full bg-[var(--color-atelier-grafite)]/5 flex items-center justify-center mb-6 group-hover:scale-110 transition-transform">
                        ...
                        <span className="font-roboto text-[13px] font-bold uppercase tracking-widest text-[var(--color-atelier-grafite)] group-hover:text-[var(--color-atelier-terracota)]">Solte a referência aqui</span>
                      </>
                    )}
                  </label>
                  ...
                  {synapses.length > 0 && (
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-4">
                      {synapses.map((syn, idx) => (
                        <div key={idx} className="glass-panel bg-white/90 p-5 rounded-[2.5rem] border border-white shadow-sm flex gap-5 items-center">
                          ...
                          <div className="flex flex-col flex-1 pr-4">
                            ...
                            <p className="text-[12px] text-[var(--color-atelier-grafite)] leading-relaxed italic line-clamp-2">"{syn.reason}"</p>
                          </div>
                        </div>
                      ))}
                    </div>
                  ...
                  <button onClick={generateSourceCode} disabled={isProcessing} className="w-full py-5 bg-[var(--color-atelier-grafite)] text-white rounded-2xl font-bold uppercase tracking-widest text-[11px] shadow-xl hover:bg-[var(--color-atelier-terracota)] hover:-translate-y-0.5 transition-all flex items-center justify-center gap-3 mt-auto disabled:opacity-50">
                ...
                <AnimatePresence>
                  {pendingSynapseIndex !== null && (
                    <div className="fixed inset-0 z-50 flex items-center justify-center px-4">
                      ...
                      <motion.div initial={{ scale: 0.9, opacity: 0, y: 20 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.9, opacity: 0, y: 20 }} className="bg-white p-8 md:p-10 rounded-[3rem] shadow-2xl relative z-10 max-w-lg w-full border border-white/20">
                        <div className="flex items-center gap-3 mb-6">
                          <div className="w-10 h-10 rounded-full bg-[var(--color-atelier-terracota)]/10 text-[var(--color-atelier-terracota)] flex items-center justify-center"><Target size={18}/></div>
                          <h3 className="font-elegant text-3xl text-[var(--color-atelier-grafite)]">O Gatilho Visual</h3>
                        </div>
                        <p className="font-roboto text-[13px] text-[var(--color-atelier-grafite)]/70 mb-6 leading-relaxed">
                        ...
                        <div className="flex gap-6 mb-8">
                          <div className="w-24 h-24 rounded-2xl overflow-hidden shadow-inner shrink-0 border border-gray-100">
                          ...
                          <textarea 
                            ...
                            className="w-full bg-[var(--color-atelier-creme)]/50 border border-[var(--color-atelier-grafite)]/10 rounded-2xl p-4 text-[13px] text-[var(--color-atelier-grafite)] resize-none h-24 outline-none focus:border-[var(--color-atelier-terracota)]/50 shadow-inner custom-scrollbar"
                          />
                        </div>
                        <button onClick={submitSynapseReason} disabled={tempReason.trim().length === 0} className="w-full py-4 bg-[var(--color-atelier-terracota)] text-white rounded-xl font-bold uppercase tracking-widest text-[10px] shadow-md hover:bg-[#8c562e] transition-colors disabled:opacity-50">
                          Registrar Visão
                        </button>
                      </motion.div>
                    </div>
            ...
            {activeZone === 5 && (
              <motion.div key="z5" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="max-w-3xl mx-auto flex flex-col gap-8 pb-10 min-h-full justify-center">
                <div className="glass-panel bg-[var(--color-atelier-grafite)] p-12 rounded-[3.5rem] text-center shadow-[0_30px_60px_rgba(0,0,0,0.2)] relative overflow-hidden border border-gray-800">
                  <div className="absolute top-0 right-0 w-64 h-64 bg-white/5 rounded-full blur-3xl"></div>
                  <div className="absolute bottom-0 left-0 w-40 h-40 bg-[var(--color-atelier-terracota)]/10 rounded-full blur-2xl"></div>
                  
                  <div className="w-24 h-24 rounded-full bg-white/5 border border-white/10 flex items-center justify-center mx-auto mb-8 shadow-inner relative z-10">
                    <CheckCircle2 size={40} className="text-[var(--color-atelier-terracota)]" />
                  </div>
                  
                  <h2 className="font-elegant text-5xl text-white mb-4 relative z-10">Código-Fonte <span className="text-[var(--color-atelier-terracota)] italic">Extraído.</span></h2>
                  <p className="font-roboto text-[14px] text-gray-400 leading-relaxed max-w-lg mx-auto mb-10 relative z-10">
                  ...
                  <div className="bg-black/30 backdrop-blur-xl p-8 rounded-3xl border border-white/5 text-left mb-10 max-h-[35vh] overflow-y-auto custom-scrollbar relative z-10 shadow-inner">
                    <div className="flex items-center gap-2 mb-4 pb-4 border-b border-white/10">
                      <BrainCircuit size={16} className="text-[var(--color-atelier-terracota)]" />
                      <span className="font-roboto text-[9px] font-bold uppercase tracking-widest text-gray-400">Atelier Core AI</span>
                    </div>
                    <div className="text-[13px] text-gray-300 leading-relaxed whitespace-pre-wrap font-medium">
                      {aiSourceCode || "Ocorreu um erro ao renderizar o código-fonte."}
                    </div>
                  </div>

                  <button onClick={generatePDF} className="bg-[var(--color-atelier-terracota)] hover:bg-white hover:text-[var(--color-atelier-grafite)] text-white px-10 py-5 rounded-2xl font-roboto text-[11px] font-bold uppercase tracking-widest transition-all shadow-xl flex items-center justify-center gap-3 mx-auto relative z-10 hover:-translate-y-1">
